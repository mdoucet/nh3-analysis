...
initial_data_file = os.path.join(ar_dir, 'REFL_201341_combined_data_auto.txt')
initial_data = np.loadtxt(initial_data_file).T

final_data_file = os.path.join(ar_dir, 'REFL_201349_combined_data_auto.txt')
final_data = np.loadtxt(initial_data_file).T

initial_err_file = os.path.join(dyn_model_dir, '201341', '__model-err.json')
initial_expt_file = os.path.join(dyn_model_dir, '201341', '__model-expt.json')

final_err_file = os.path.join(dyn_model_dir, '201349', '__model-err.json')
final_expt_file = os.path.join(dyn_model_dir, '201349', '__model-expt.json')

dynamic_run = 201348
store_basename = os.path.join(dyn_model_dir, '%s-dyn/results-30s-bck-mar2024' % dynamic_run)

results_dir = os.path.join(dyn_model_dir, store_basename)


loop = fitting_loop.FittingLoop(dyn_data_dir, results_dir=results_dir, model_dir=project_dir, model_name='model-loop-201348',

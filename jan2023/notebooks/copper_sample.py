...
class Measurement_CuB_OCV2(Measurement):
    def get_sample(self):
        # Materials ####################################################################
        Si = SLD(name='Si', rho=2.07, irho=0.0)
        THF = SLD(name='THF', rho=self.thf_sld, irho=0.0)
        siox = SLD(name='siox', rho=2.8, irho=0.0)
        Ti = SLD(name='Ti', rho=-1.2, irho=0.0)
        Cu = SLD(name='Cu', rho=self.cu_sld, irho=0.0)
        if self.fit_abs:
            material = SLD(name='material', rho=0.9, irho=0.1)
        else:
            material = SLD(name='material', rho=2, irho=0.0)
        SEI = SLD(name='SEI', rho=4.2, irho=0.00)

        # Film definition ##############################################################
        sample = (  THF(0, 54.16) | SEI(171, 13) | material(37, 9.74) | Cu(565.6, 10.24) | Ti(51, 11.1) | siox(32, 2.92) | Si )
        return sample
    ...
    def set_ranges(self):
        ...
        sample['siox'].interface.range(1.0, 20.0)
        sample['siox'].material.rho.range(-1.3, 4.0)
        sample['Ti'].thickness.range(10.0, 70.0)
        sample['Ti'].interface.range(1.0, 9.0)
        sample['Ti'].material.rho.range(-4, -1)
        sample['Cu'].thickness.range(10.0, 800.0)
        sample['Cu'].interface.range(5.0, 22.0)
        #sample['Cu'].material.rho.range(6.3, 7.0)
